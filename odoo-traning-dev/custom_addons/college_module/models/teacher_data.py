...
class TeacherData(models.Model):
    ...
    def _total_course_price(self):
        """function to calculate the total course price"""
        for record in self:
            record.total_amount = record.course_price

    def _inverse_total_price(self):
        """function to calculate the total course price used to set in the one2many"""
        for record in self:
            record.course_price = record.total_amount

    @api.onchange('teacher_department')
    def _onchange_teacher_department(self):
        """function to set the teacher department onchange of teacher department"""
        for record in self:
