...
class PatientAmbulatory(models.Model):
    ...
    procedure_ids = fields.Many2many("ambulatory.care.procedure")

    @api.onchange("patient_id")
    def _onchange_patient_id(self):
        for record in self:
            if record.patient_id:
                record.health_proffessional = record.patient_id.patient_primary_care_doctor
                record.ordering_physician = record.patient_id.patient_primary_care_doctor
                record.related_evaluation = record.patient_id.patient_primary_care_doctor
            else:
