...
class Insaurances(models.Model):
    ...
    insaurance_company = fields.Many2one("insaurance.company",string="Insaurance Company",required=True)
    expiration_date = fields.Date(string="Expiration Date")

    @api.model
    def create(self, vals):
        """Automatically generate a reference number for new insausrance."""
        vals['nummber'] = self.env['ir.sequence'].next_by_code('insaurances.insaurances')
