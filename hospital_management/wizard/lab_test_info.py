...
class LabTestInfo(models.Model):
    ...
    test_id = fields.Char(default=lambda self: ('New'),readonly=True, copy=False,string="Id")
    ...
    def create(self, vals):
        """Automatically generate a reference number for new lab test result."""
        vals['test_id'] = self.env['ir.sequence'].next_by_code('lab.test.info')
        return super(LabTestInfo, self).create(vals)
    ...
    def _onchange_patient_id(self):
        ...
        for record in self:
            if record.patient_id:
                ...
                record.test_type = patient_test_type.test_type
